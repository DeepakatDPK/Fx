...
class DayTraderAgent(BaseAgent):
    def __init__(self,
        ...
        self.log(f"{self.agent_id} initialized with broker. Timeframe: {self.timeframe}, Bars: {self.num_bars_to_fetch}, Strategy Params: EMA({self.ema_short_period}/{self.ema_long_period}), RSI({self.rsi_period}), MACD({self.macd_fast}/{self.macd_slow}/{self.macd_signal})")
        self.log(f"Broker type: {type(self.broker)}")
    ...
    def _get_timeframe_seconds_approx(self, timeframe_str: str) -> int:
        ...
        self.log(f"Warning: Unknown timeframe '{timeframe_str}', defaulting to 1 hour for duration calculation.")
    ...
    def process_task(self, state: Dict, mode: str = "quick") -> Dict:
        ...
        try:
            ...
            if not task:
                self.log(f"{self.agent_id}: No current_day_trader_task found in state.")
            ...
            if not current_simulated_time_iso:
                ...
                self.log(f"{self.agent_id}: Using broker: {self.broker}, Timeframe: {self.timeframe}, Bars to fetch: {self.num_bars_to_fetch}")
                ...
                try:
                    ...
                    self.log(f"{self.agent_id}: Requesting historical data for {currency_pair} from {datetime.datetime.fromtimestamp(start_historical_data_request_unix, tz=datetime.timezone.utc)} to {datetime.datetime.fromtimestamp(end_historical_data_request_unix, tz=datetime.timezone.utc)}")
            ...
            if historical_data and len(historical_data) >= self.ema_long_period: # Check if enough data for longest EMA
                try:
                    ...
                    self.log(f"{self.agent_id}: Calculating TA indicators...")
            ...
            if not latest_indicators or not indicators_present:
                ...
                self.log(f"{self.agent_id}: Skipping strategy rules due to missing indicators. {latest_indicators}")
            else:
            ...
            self.log(f"{self.agent_id}: Strategy decision: {final_signal}, Confidence: {final_confidence}")
            ...
            result = {"day_trader_proposal": trade_proposal}
            self.log(f"Decision: {result}")
            return result
        except Exception as e:
            self.log(f"Error: {e}")
            return {"error": str(e)}
